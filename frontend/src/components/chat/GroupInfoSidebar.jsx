...
import { 
  ...
  Edit2, Camera, Check as CheckIcon, Info 
} from "lucide-react";
...
export default function GroupInfoSidebar({ onClose }) {
  const { 
    selectedGroup, removeMember, leaveGroup, updateMemberRole, 
    getGroupSharedMedia, updateGroupInfo 
  } = useGroup();
  ...
  const handleLeaveGroup = () => {
    const msg = isOwner 
      ? "As owner, you must delete the group or transfer ownership instead of leaving." 
      : "Are you sure you want to leave this group?";
    
    if (isOwner) return toast.error(msg);
                  ...
                  <div key={userId} className="flex items-center justify-between p-2 rounded-lg hover:bg-gray-50 group relative transition-colors">
                    ...
                    {canInteract && (
                        <div className="relative">
                            ...
                            {activeMenuId === userId && (
                                <div className="absolute right-0 top-8 w-44 bg-white border border-gray-100 shadow-xl rounded-lg z-50 overflow-hidden animate-in fade-in zoom-in-95 duration-100">
                                    {isOwner && role !== 'admin' && (
