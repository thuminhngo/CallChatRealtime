...
// Thêm prop showSenderName và senderName
export default function MessageBubble({ message, isMe, avatar, isLastInGroup, showSenderName, senderName }) {
  ...
  return (
    <div className={`flex items-end mb-2 gap-2 group relative ${isMe ? "flex-row-reverse" : "flex-row"}`} ref={wrapperRef}>
      ...
      <div className={`flex flex-col ${isMe ? "items-end" : "items-start"} max-w-[70%] relative`}>
        ...
        {/* 🔥 FIX: Hiển thị tên người gửi trong nhóm */}
        {showSenderName && !isMe && (
        ...
        <div className={`text-[14.5px] break-words relative z-0
          ...
          {/* Logic hiển thị Attachments (cho Group) hoặc Image/Audio cũ (cho 1-1) */}
             ...
             message.attachments.map((att, index) => (
                att.type === 'image' ? (
                  <img
                    ...
                    src={att.url}
                    alt="Shared image"
                    className="object-cover rounded-lg mb-2 cursor-pointer border border-white/20"
                    width={360}
                    height={360}
                    onClick={() => window.open(att.url, '_blank')}
                  />
                ) : att.type === 'audio' ? (
                   <div key={index} className={`flex items-center gap-2 w-full min-w-[200px] sm:min-w-[280px] max-w-full ${isMe ? "text-white" : "text-gray-800"}`}>
