...
export default function SentRequests() {
    ...
    // SỬA: Truy cập trực tiếp user.fullName
    ...
    return (
        <div className={viewMode === 'grid' ? "grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4 pb-4 pr-2" : "flex flex-col gap-3 pb-4 pr-2"}>
            {filteredList.map((user) => (
                <div key={user._id} className={`bg-white rounded-2xl border border-gray-100 shadow-sm flex items-center gap-4 p-3 ${viewMode === 'grid' ? 'flex-col text-center sm:flex-row sm:text-left' : ''}`}>
                    ...
                    <button
                        ...
                        className="p-2 bg-gray-50 text-gray-400 hover:text-red-500 hover:bg-red-50 rounded-xl transition-colors"
                    >
