...
import { useAuth } from "../context/AuthContext";
import { useSocket } from "../context/SocketContext";
import { PhoneOff, Mic, MicOff, Video, VideoOff, User, Clock } from "lucide-react"; // Thêm Clock icon
import { axiosInstance } from "../lib/axios";
...
export default function CallPage() {
  ...
  // Dùng Ref để lưu giá trị timer và trạng thái kết nối
  const timerRef = useRef(0);
  const hasAnsweredRef = useRef(false); // Quan trọng: Check xem đối phương đã bắt máy chưa
  const localVideoRef = useRef(null);
  ...
  // --- 1. KHỞI TẠO CUỘC GỌI (Logic giữ nguyên) ---
  useEffect(() => {
    const initCall = async () => {
      if (!authUser || !socket || !channelName) return;
    ...
    // Lắng nghe sự kiện kết thúc từ Server (do bên kia tắt hoặc server xử lý)
    if(socket) {
      socket.on("call:ended", () => {
          // Alert nhẹ hoặc đóng luôn
          window.close();
      });
      socket.on("callCancelled", () => {
  ...
  useEffect(() => {
    ...
    if (remoteUsers.length > 0) {
        hasAnsweredRef.current = true; // Đánh dấu đã bắt máy
        setStatusText("Đang trò chuyện");
        
        const interval = setInterval(() => {
            setTimer((prev) => {
                const next = prev + 1;
                timerRef.current = next; // Update ref để dùng lúc End Call
                return next;
            });
        }, 1000);
        return () => clearInterval(interval);
  ...
  const handleEndCall = () => {
     if (socket) {
        ...
        // Nếu hasAnsweredRef = true -> Đã nói chuyện -> Status: "answered"
        // Nếu hasAnsweredRef = false -> Chưa ai nghe mà đã tắt -> Status: "missed"
     ...
     // Dọn dẹp phía Client ngay lập tức
     leaveChannel();
     window.close();
  };
